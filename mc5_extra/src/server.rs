...
impl Mc5 for Mc5Server {
    ...
    #[instrument(skip(self))]
    async fn get_label_docs(
        self,
        _context: tarpc::context::Context,
        bucket: String,
        label: Label,
    ) -> Result<Vec<u128>, Mc5Error> {
        let bucket = self.db.get_bucket(&bucket)?;
